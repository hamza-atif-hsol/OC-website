...
export default function CorporateInstitutionalBankingPage() {
  const { t } = useLanguage()
  ...
  const heroBullets: string[] = [
    "Digitize onboarding and lifecycle journeys for corporate and institutional clients at scale.",
    "Accelerate onboarding for low-to-medium risk clients, while managing higher-risk accounts by exception.",
    "Stay ahead of evolving regulatory requirements with consistent, policy-driven controls.",
  ]

  const helps: HelpItem[] = [
    {
      icon: ShieldCheck,
      title: "Reduce risk and strengthen regulatory compliance",
      desc:
        "Apply consistent, policy-driven checks across jurisdictions with complete auditability   reducing exposure while improving control.",
    },
    {
      icon: Timer,
      title: "Faster onboarding for corporate and institutional clients",
      desc:
        "Reduce friction and handoffs with orchestration and straight-through processing   helping clients get to revenue sooner.",
    },
    {
      icon: Workflow,
      title: "Transform the end-to-end client experience",
      desc:
        "Deliver a connected, digital journey across onboarding, reviews, and refresh cycles   minimizing rework and repeat outreach.",
    },
  ]
  ...
  const exploreItems: ExploreItem[] = [
    ...
    },
    {
      ...
      description:
        "Digitalize end-to-end KYC across the client lifecycle simplifying due diligence with rules, workflows, and continuous risk monitoring.",
  ...
  const partners: Partner[] = [
    ...
    // { name: "Luxoft", logo: "/partners/luxoft.png" },
    // { name: "Jumio", logo: "/partners/jumio.png" },
    // { name: "Moody's", logo: "/partners/moodys.png" },
  ...
  const insights = React.useMemo(
    () => [
      {
        title: "Corporate onboarding: 5 ways to reduce friction",
        desc: "Practical ways to reduce handoffs, rework, and time-to-onboard while strengthening controls.",
        href: "/resources/insights/corporate-onboarding",
      },
      {
        title: "Policy-driven controls across jurisdictions",
        desc: "How to standardize compliance while supporting regional and line-of-business variation.",
        href: "/resources/insights/policy-controls",
      },
      {
        title: "Agentic AI for compliance teams",
        desc: "Where intelligent agents can automate repeatable work safely, with governance built in.",
        href: "/resources/insights/agentic-ai",
      },
    ],
    []
  )
  ...
  const demoPoints: string[] = [
    "Improve operational efficiency across onboarding and reviews",
    "Strengthen financial crime and regulatory compliance controls",
    "Transform the onboarding and lifecycle experience",
    "Unlock revenue faster with reduced time-to-onboard",
  ]
  ...
  return (
    <main className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="space-y-6">
              <motion.div variants={fadeInUp} className="text-sm text-slate-500">
                Homepage / Segments / Corporate & Institutional Banking
              </motion.div>
              ...
              <motion.p variants={fadeInUp} className="text-base md:text-lg text-slate-600 leading-relaxed max-w-xl">
                One Constellation Client Lifecycle Management enables banks to modernize corporate and institutional onboarding and lifecycle operations   improving compliance, reducing friction, and boosting operational efficiency.
              </motion.p>
            ...
            <motion.div
              ...
              className="relative"
            >
              <div className="relative rounded-3xl overflow-hidden border border-slate-200 bg-slate-50 shadow-sm h-[450px]">
                <img
                  src="/corporate-institutional-banking.jpg"
                  alt="Corporate & Institutional Banking"
              ...
              <div className="hidden md:block absolute -left-6 bottom-10 rounded-2xl bg-white shadow-lg border border-slate-200 p-4 w-[260px]">
                ...
                <p className="text-sm text-slate-600 mt-2">
                  Centralize workflows, documents, and approvals into a single controlled process for corporate clients.
                </p>
              </div>
            </motion.div>
          </div>
      ...
      {/* ✅ FULL WIDTH + CENTERED LOGOS (no text, no border) */}
        ...
        <div className="container mx-auto px-4">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className=" mx-auto">
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl font-bold text-slate-900">
              How One Constellation Helps Corporate & Institutional Banks
            </motion.h2>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className=" mx-auto">
                  ...
                  >
                    <div className="flex items-start justify-between gap-6">
                      <div className="space-y-3">
                        <h3 className="text-2xl font-bold text-slate-900">{activeExplore.title}</h3>
            ...
            <motion.div variants={fadeInUp} className="mt-10 rounded-3xl bg-accent/10 border border-accent/15 px-6 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div className="flex gap-2 md:self-auto self-end">
                <button type="button" onClick={prevInsight} aria-label="Previous insight" className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button type="button" onClick={nextInsight} aria-label="Next insight" className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50">
        ...
        <div className="container mx-auto px-4">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className=" mx-auto">
            ...
            <motion.p variants={fadeInUp} className="text-white/70 mt-4 max-w-3xl">
              We partner with market-leading providers to help you implement faster, enrich data quality, and connect verification, screening, and workflow into one lifecycle experience.
            </motion.p>
            ...
            <motion.div variants={fadeInUp} className="mt-10 flex flex-wrap gap-4">
              {partners.map((p) => (
                <div key={p.name} className="h-12 px-5 rounded-2xl bg-white/95 flex items-center justify-center border border-white/20">
                  {p.logo ? <img src={p.logo} alt={p.name} className="h-6 w-auto object-contain" /> : <span className="text-sm font-semibold text-slate-800">{p.name}</span>}
                </div>
              ))}
            </motion.div>
          </motion.div>
              ...
              <div className="flex items-start justify-between gap-6">
                <div className="space-y-4">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    “One Constellation helped us simplify corporate onboarding by bringing policy-driven controls, workflow automation, and real-time visibility into a single lifecycle experience   improving both compliance outcomes and client satisfaction.”
                  </p>
                  <div className="text-sm text-slate-600">  Head of CIB Operations, Global Bank</div>
                </div>
      ...
      <section className="relative overflow-hidden py-20 md:py-24 fintech-gradient">
        <div className="container mx-auto px-4">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className=" mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <motion.div variants={fadeInUp} className="lg:col-span-7">
              <Link href="/book-demo" className="inline-flex items-center rounded-full bg-slate-900 text-white px-6 h-12 text-lg font-semibold hover:bg-slate-800 transition">
                Book a Demo
              </Link>
            ...
            <motion.div variants={fadeInUp} className="lg:col-span-5">
              <form onSubmit={submitDemo} className="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <input className="h-11 rounded-xl border border-slate-200 px-4 outline-none" placeholder="First name" value={demoForm.firstName} onChange={(e) => onChangeDemo("firstName", e.target.value)} />
                  <input className="h-11 rounded-xl border border-slate-200 px-4 outline-none" placeholder="Last name" value={demoForm.lastName} onChange={(e) => onChangeDemo("lastName", e.target.value)} />
                  <input className="h-11 rounded-xl border border-slate-200 px-4 outline-none" placeholder="Job title" value={demoForm.jobTitle} onChange={(e) => onChangeDemo("jobTitle", e.target.value)} />
                  <input className="h-11 rounded-xl border border-slate-200 px-4 outline-none" placeholder="Company" value={demoForm.company} onChange={(e) => onChangeDemo("company", e.target.value)} />
                  <input className="h-11 rounded-xl border border-slate-200 px-4 outline-none" placeholder="Work email" value={demoForm.workEmail} onChange={(e) => onChangeDemo("workEmail", e.target.value)} />
                </div>
