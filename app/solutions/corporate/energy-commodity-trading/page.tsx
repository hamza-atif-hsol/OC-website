...
import { ArrowRight, Building2, Star, ChevronLeft, ChevronRight, Users, Layers, Users2, TrendingUp, Zap } from "lucide-react"
...
function TrustedOrganizationsMarquee() {
  const logos: TrustedLogo[] = [
    { name: "Boerse Stuttgart Digital", src: "/boerse-stuttgart-digital.svg" },
    { name: "Moeve", src: "/moeve.svg" },
    { name: "RWE", src: "/rwe.svg" },
  ]
...
const HelpItemIcon = ({ icon: Icon }: { icon: React.ComponentType<{ className?: string }> }) => (
  <Icon className="w-5 h-5 text-accent" />
)

export default function EnergyCommodityTradingPage() {
  const breadcrumbText = "Homepage / Segments / Energy and Commodity Trading"

  const heroTitleLines = ["Energy &", "Commodity Trading"]

  const heroDescription =
    "One Constellation empowers energy and commodity trading firms to streamline counterparty management, accelerate onboarding, and maintain robust compliance across complex trading operations."

  const heroBullets: string[] = [
    "Rapid counterparty and trading partner onboarding with compliance automation.",
    "Centralized trader and account management across multiple trading venues.",
    "Advanced transaction monitoring and financial crime controls for trading operations.",
  ]

  const helps: HelpItem[] = [
    {
      icon: Zap,
      title: "Streamlined counterparty management",
      desc: "Manage complex counterparty onboarding and relationship lifecycle with centralized workflows, document management, and approval routing.",
    },
    {
      icon: TrendingUp,
      title: "Rapid market entry",
      desc: "Accelerate new counterparty activation and trading relationships with automated KYC and compliance workflows.",
    },
    {
      icon: Users2,
      title: "Trading compliance at scale",
      desc: "Maintain consistent transaction monitoring and financial crime controls across all trading activities and counterparties.",
    },
  ]
  ...
  const insights = React.useMemo(
    () => [
      {
        title: "Counterparty management in trading",
        desc: "Best practices for rapid counterparty onboarding without compromising compliance.",
        href: "/resources/insights/counterparty-trading",
      },
      {
        title: "Transaction monitoring for energy trading",
        desc: "Managing financial crime risks across volatile commodity and energy trading.",
        href: "/resources/insights/trading-monitoring",
      },
    ],
    []
  )
  ...
  const demoPoints: string[] = [
    ...
    "Reduce compliance friction in trading operations",
    "Strengthen transaction monitoring controls",
    "Scale trading activities with confidence",
  ]

  const testimonialQuote =
    "One Constellation streamlined our counterparty management and compliance processes enabling us to on board new trading partners faster while maintaining rigorous compliance standards across all venues."

  const testimonialAttribution = "Head of Compliance, Global Energy Trading Firm"
  ...
  return (
    <main className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="space-y-6">
              ...
              <motion.div variants={fadeInUp} className="pt-2 flex flex-wrap gap-3">
                <Button asChild className="rounded-full px-6 h-11">
                  <Link href="/resources/brochure">Download the brochure</Link>
                </Button>
              </motion.div>
            </motion.div>
            ...
            >
              <div className="relative rounded-3xl overflow-hidden border border-slate-200 bg-slate-50 shadow-sm">
                <img src="/segment-cib.png" alt="Energy Trading" className="w-full h-auto object-contain" />
              </div>
              ...
              <div className="hidden md:block absolute -left-6 bottom-10 rounded-2xl bg-white shadow-lg border border-slate-200 p-4 w-[260px]">
                ...
                <p className="text-sm text-slate-600 mt-2">
                  Manage counterparty relationships and trading compliance with centralized workflows.
                </p>
              </div>
            </motion.div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="mx-auto">
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl font-semibold text-slate-900">
              How One Constellation Helps Energy & Trading Firms
            </motion.h2>
      ...
      {/* PARTNERS */}
      <section className="py-20 md:py-24 bg-gradient-to-b from-[#002E49] via-slate-900 to-slate-800 border-t border-slate-800">
        <div className="container mx-auto px-4">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="mx-auto">
            ...
            <motion.p variants={fadeInUp} className="text-white/70 mt-4 max-w-3xl">
              We partner with market-leading providers to help you implement faster and deliver robust trading compliance solutions.
            </motion.p>
      ...
      {/* CONTACT US */}
      <section className="relative overflow-hidden py-20 md:py-24 bg-gradient-to-r from-accent/10 to-accent/5">
        <div className="container mx-auto px-4">
          <motion.div
            initial="hidden"
            whileInView="visible"
